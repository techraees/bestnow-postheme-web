    ...
    <div className="h-[615px] p-2 bg-white dark:bg-dark_mode_primary_overlay_5 flex flex-col justify-between">
        ...
        <div>
          {[...Array(9)].map((_, index) => (
            <div
              ...
              className={`h-[10px] ${
                index === 8 ? "w-[30%]" : "w-[100%]"
              } rounded-full animate-pulse mb-1.5 bg-light_mode_primary_1 dark:bg-dark_mode_primary_overlay_16`}
            />
          ))}
        </div>
      </div>
      <div className="flex justify-between mt-1 mb-1">
        <div className="flex gap-3">
          {[...Array(3)].map((_, index) => (
            <div
              key={index}
              className="h-[28px] w-[28px] rounded-full animate-pulse mb-1.5 bg-light_mode_primary_1 dark:bg-dark_mode_primary_overlay_16"
            />
          ))}
        </div>
      ...
      <div className="flex justify-end gap-3">
        {[...Array(2)].map((_, index) => (
          <div
            key={index}
            className="h-[14px] w-[70px] rounded-full animate-pulse mb-1.5 bg-light_mode_primary_1 dark:bg-dark_mode_primary_overlay_16"
          />
        ))}
      </div>
    </div>
